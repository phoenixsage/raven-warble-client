        ...
        >
            <div className="grid grid-cols-3">
                <div className="flex flex-col">
                    <div><span className="font-bold text-blue-700 text-xs">ACC: {transaction.accountNumber}</span></div>
                    <div><span className="font-bold text-gray-700 text-xs">From: </span>{transaction.sender}</div>
                    <div><span className="font-bold text-gray-700 text-xs">Sender Bank: </span>{transaction.senderBank}</div>
                    <div><span className="font-bold text-gray-700 text-xs">Sender Acc: </span>{transaction.senderAcc}</div>
                    <div><span className="font-bold text-gray-700 text-xs">SessionId: </span><span className="text-sm">{transaction.sessionId}</span></div>
                </div>
                <div>
            ...
            <div className={`absolute -right-16 rotate-45  ${transaction.status === 'complete'?'bg-green-600': transaction.status === 'incomplete'?'bg-yellow-600':'bg-red-600'} w-56  shadow top-5 text-center`}>
